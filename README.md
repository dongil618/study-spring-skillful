...
- jwt 적용
- 회원가입 validation 에러처리만 했고 메시지 처리(buildresult) 못했음.
- 댓글 조회(테이블 조인해서 해당 포스팅 글의 댓글들을 불러오는 작업)
- 댓글 수정
- 댓글 삭제
- 테스트코드 작성
- 자신의 글이나 자신의 댓글에만 삭제, 편집 버튼 나오게.

### 오류

- 로그인하면 localhost:8080/main.js로 redirect됨.
- db에서 created_at 값이 넘어오지 않음.
...
2. 회원 가입 API
    ...
    - 데이터베이스에 존재하는 닉네임을 입력한 채 회 원가입 버튼을 누른 경우 "중복된 닉네임입니다." 라는 에러메세지를 response에 포함하기
3. 로그인 API
